...
def deduplicate_evidence_sources(
    ...
    for evidence_source in evidence_sources:
        ...
        #print("searching qdrant for evidence sources")
        # print(
    ...
    async def process_requirement(
        ...
        *,
        ...
        async with semaphore:
            ...
            # Build the exact prompt that will be sent to the LLM.
            ...
            # This is only an estimate. Actual token usage is read from
            # the LLM response below.
            estimated_input_tokens = max(
                1,
                round(
                    prompt_character_count / 4
                ),
            )

            retrieved_source_ids = normalize_string_list(
                [
                    evidence_source.get(
                        "EvidenceId",
                        "",
            ...
            # Print the complete LLM input only for the first requirement.
            # Remove this block after testing because the prompt may contain
            # confidential company evidence.
            if requirement_number == 1:
                print(
                    "\n"
                    "========== EVIDENCE LLM INPUT START =========="
                    "\n"
                )
                # print(prompt)
                print(
                    "\n"
                    "========== EVIDENCE LLM INPUT END =========="
                    "\n"
                )
            ...
            if (
                evidence_found
                and not selected_evidence_sources
            ):
                invalid_positive_reasons.append(
            ...
            if evidence_found and (
                ...
                invalid_positive_reasons.append(
                    "The positive decision relied on "
            ...
            if invalid_positive_reasons:
                ...
                if not missing_evidence_reason:
                    missing_evidence_reason = (
                        "Direct, specific and verifiable "
                        "company evidence supporting the "
                        "complete requirement is missing."
                    )
                ...
                if not evidence_reason:
                    evidence_reason = (
                        "The selected company evidence "
                        "directly supports the complete "
                        "requirement."
                    )
                ...
                if not evidence_reason:
                    evidence_reason = (
                        "The retrieved chunks do not "
                        "provide direct and complete "
                        "support for the requirement."
                    )

                if not missing_evidence_reason:
                    missing_evidence_reason = (
                        "A direct policy, process, "
                        "certification, control, "
                        "commitment, methodology or "
                        "verifiable record supporting "
