...
def build_deduplication_prompt(
    ...
    """
    Build a compact requirement-deduplication prompt.

    Important:
    - Do not send the complete OriginalRequirement object
      to the LLM because RequirementText already contains
      the material requirement.
    - The full original requirement objects remain available
      in raw_requirements and are restored by
      validate_deduplication_result().
    - Only operational identifiers needed for traceability
      are included from context.
    """
    ...
    for index, requirement in enumerate(
        ...
        prompt_requirement: dict[str, Any] = {
            "RequirementIndex": index,
            "RequirementId": get_requirement_id(
                requirement,
                index,
        ...
        # Preserve only small fields that can materially
        # affect deduplication. Do not include the complete
        # MongoDB requirement object in the LLM prompt.
        if isinstance(requirement, Mapping):
            optional_fields = (
                "RequirementType",
                "MandatoryFlag",
    ...
    # Large sourceIds arrays and other operational metadata
    # do not help the LLM decide semantic duplication.
    # Keep only small traceability values.
    ...
    for key_group in context_key_groups:
...
""".strip()
    ...
    # Safe diagnostics only. Requirement content is not printed.
    print(
        "Deduplication prompt prepared:",
        {
            ...
            "compactContextKeys": list(
                compact_context.keys()
            ),
        },
    )
